...
genotyped_individuals = genotypes %>% 
  ...
  mutate( ## we take the chance to mutate the genotypes now 
    genotype = case_when(
      genotype == "0/0" ~ 0, #specific genotypes whith huypothesis
      genotype == "0/1" ~ 1,
      genotype == "1/1" ~ 2,
      TRUE ~ NA
    ) 
...
### tidymodels approach, PCA can be performed within the recipe through step_
...
pca_transformation <- pca_recipe %>% 
  step_normalize(all_numeric()) %>% #scale and centering
  step_pca(all_numeric(), num_comp = 10) #here s the one
...
pca_estimates <- prep( #make the estimation, you pass the recipe and choose the dataset you pass to training
  pca_transformation,
...
pca_data <- bake(
  pca_estimates,
  genotyped_individuals %>% select(-c(individual)) #projection of you data on your new space
  )

### this can be plotted as before
ggplot(pca_data,
       aes(x=PC01, y=PC02))+ #variables of pca_data tibble
  geom_point()
#you see how there s no correlation, you started from a 5k individuals and 2k genotypes, and you represented all of them in this plot
#the new axes are not correlated with each other, but doesn t mean that they are associated to an outcome like the original predictors
#U MAPS are not orthogonal to each other, they retain relationships, mainly local one (like single cell type, they are represented near to each other)
